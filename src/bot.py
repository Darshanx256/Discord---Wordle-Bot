...
class WordleBot(commands.Bot):
    ...
    def setup_db(self):
        ...
        try:
            ...
            response = self.supabase_client.from_('user_stats_v2').select('user_id', count='exact').limit(1).execute()
        ...
        while not self.is_closed():
            if time.monotonic() >= next_run:
                if self.supabase_client:
                    try:
                        def ping_db_sync():
                            self.supabase_client.table('user_stats_v2').select('user_id', count='exact').limit(1).execute()
